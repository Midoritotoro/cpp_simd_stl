...
int main() {
...

    {
        ...
        simd_stl::concurrency::this_thread::sleep_for(std::chrono::milliseconds(50));
        assert(flag.load() == true);
    }

    {
        std::atomic<bool> flag1{ false }, flag2{ false };
        simd_stl::concurrency::thread t1, t2;
    ...
    {
        auto hc = simd_stl::concurrency::thread::hardwareConcurrency();
        assert(hc > 0);
