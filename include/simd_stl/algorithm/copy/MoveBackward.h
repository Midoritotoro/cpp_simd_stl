...
simd_stl_constexpr_cxx20 simd_stl_always_inline _BidirectionalSecondIterator_ move_backward(
    ...
    _BidirectionalSecondIterator_   destinationLast) noexcept
{
    ...
    if constexpr (type_traits::IteratorCopyCategory<_BidirectionalFirstIterator_, _BidirectionalSecondIterator_>::BitcopyAssignable) {
        ...
        _MemmoveVectorized(destinationLastChar - byteLength, firstAddress, byteLength);
        ...
        while (firstUnwrapped != lastUnwrapped)
            *(--destinationLastUnwrapped) = std::move(*(--lastUnwrapped));
...
simd_stl_constexpr_cxx20 simd_stl_always_inline _BidirectionalSecondIterator_ move_backward(
    ...
    _BidirectionalSecondIterator_   destinationLast) noexcept
{
    return simd_stl::algorithm::move_backward(first, last, destinationLast);
}
