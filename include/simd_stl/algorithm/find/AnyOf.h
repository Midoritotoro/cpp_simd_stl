...
template <
    ...
    class _Predicate_>
_Simd_inline_constexpr bool any_of(
...
{
    return (simd_stl::algorithm::find_if(_First, _Last, type_traits::passFunction(_Predicate)) != _Last);
}
...
simd_stl_nodiscard simd_stl_constexpr_cxx20 simd_stl_always_inline bool any_of(
    ...
    _Predicate_     _Predicate) noexcept(
        type_traits::is_nothrow_invocable_v<
            _Predicate_, type_traits::IteratorValueType<_InputIterator_>>)
{
    return simd_stl::algorithm::any_of(_First, _Last, type_traits::passFunction(_Predicate));
}
