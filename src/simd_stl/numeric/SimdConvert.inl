    ...
    _SimdConvertImplementation<arch::CpuFeature::SSE2, xmm128>::_ToMask(_VectorType_ _Vector) noexcept
{
    if      constexpr (sizeof(_DesiredType_) == 8)
        return _mm_movemask_pd(__intrin_bitcast<__m128d>(_Vector));

    else if constexpr (sizeof(_DesiredType_) == 4)
        return _mm_movemask_ps(__intrin_bitcast<__m128>(_Vector));

    else if constexpr (sizeof(_DesiredType_) == 2)
        return _mm_movemask_epi8(_mm_packs_epi16(__intrin_bitcast<__m128i>(_Vector), _mm_setzero_si128()));

    else if constexpr (sizeof(_DesiredType_) == 1)
        return _mm_movemask_epi8(__intrin_bitcast<__m128i>(_Vector));
}
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept 
{
    ...
    if constexpr (_Bits == 2) {
        ...
        return __intrin_bitcast<_VectorType_>(_mm_cmpeq_epi32(_Broadcasted, _mm_set1_epi32(1)));
    }
        ...
        return __intrin_bitcast<_VectorType_>(_mm_cmpeq_epi16(_Broadcasted, _mm_set1_epi16(1)));
    }
        ...
        return __intrin_bitcast<_VectorType_>(_mm_cmpeq_epi8(_Broadcasted, _mm_set1_epi8(1)));
    }
}
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept 
{
    ...
    if constexpr (_Bits == 2) {
        ...
        return __intrin_bitcast<_VectorType_>(_mm_cmpeq_epi8(_mm_and_si128(_Shuffled, _Select), _Select));
    }
}
    ...
    _SimdConvertImplementation<arch::CpuFeature::AVX, ymm256>::_ToMask(_VectorType_ _Vector) noexcept
{
    ...
    if      constexpr (sizeof(_DesiredType_) == 8) {
        return static_cast<_MaskType>(_mm256_movemask_pd(__intrin_bitcast<__m256d>(_Vector)));
    }
    else if constexpr (sizeof(_DesiredType_) == 4) {
        return static_cast<_MaskType>(_mm256_movemask_ps(__intrin_bitcast<__m256>(_Vector)));
    }
    else if constexpr (sizeof(_DesiredType_) == 2) {
        const auto _Zeros = _mm_setzero_si128();

        const auto _Low = _mm_movemask_epi8(_mm_packs_epi16(__intrin_bitcast<__m128i>(_Vector), _Zeros));
        const auto _High = _mm_movemask_epi8(_mm_packs_epi16(_mm256_extractf128_si256(__intrin_bitcast<__m256i>(_Vector), 1), _Zeros));

        return ((static_cast<_MaskType>(_High & 0xFF) << 8) | (static_cast<_MaskType>(_Low & 0xFF)));
    }
    else if constexpr (sizeof(_DesiredType_) == 1) {
        const auto _Low = _SimdToMask<arch::CpuFeature::SSE2, xmm128, _DesiredType_>(__intrin_bitcast<__m128i>(_Vector));
        const auto _High = _SimdToMask<arch::CpuFeature::SSE2, xmm128, _DesiredType_>(_mm256_extractf128_si256(
            __intrin_bitcast<__m256i>(_Vector), 1));
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept
{
    ...
    if constexpr (_Bits == 4) {
        ...
        return __intrin_bitcast<_VectorType_>(_mm256_cmp_pd(_Broadcasted, _mm256_set1_pd(1), 0));
    }
    else if constexpr (_Bits == 8) {
        const auto _Broadcasted = _mm256_set_ps((_Mask >> 7) & 1, (_Mask >> 6) & 1, (_Mask >> 5) & 1, (_Mask >> 4) & 1,
            (_Mask >> 3) & 1, (_Mask >> 2) & 1, (_Mask >> 1) & 1, _Mask & 1);
        return __intrin_bitcast<_VectorType_>(_mm256_cmp_ps(_Broadcasted, _mm256_set1_ps(1), 0));
    }
    else if constexpr (_Bits == 16) {
        ...
        auto _Result = __intrin_bitcast<__m256i>(_Low);
...
{
    if      constexpr (sizeof(_DesiredType_) == 8) {
        return _mm256_movemask_pd(__intrin_bitcast<__m256d>(_Vector));
    }
    else if constexpr (sizeof(_DesiredType_) == 4) {
        return _mm256_movemask_ps(__intrin_bitcast<__m256>(_Vector));
    }
    else if constexpr (sizeof(_DesiredType_) == 2) {
        const auto _Pack = _mm256_packs_epi16(__intrin_bitcast<__m256i>(_Vector), _mm256_setzero_si256());
        const auto _Shuffled = _mm256_permute4x64_epi64(_Pack, 0xD8);

        return _mm256_movemask_epi8(_Shuffled);
    }
    else if constexpr (sizeof(_DesiredType_) == 1) {
        return _mm256_movemask_epi8(__intrin_bitcast<__m256i>(_Vector));
    }
}
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept 
{
    ...
    if constexpr (_Bits == 4) {
        ...
        return __intrin_bitcast<_VectorType_>(_mm256_cmpeq_epi16(_Broadcasted, _mm256_set1_epi16(1)));
    }
    else if constexpr (_Bits == 32) {
        ...
        return __intrin_bitcast<_VectorType_>(_mm256_cmpeq_epi8(_mm256_and_si256(_Shuffled, _Select), _Select));
    }
}
#pragma endregion
    ...
    _SimdConvertImplementation<arch::CpuFeature::AVX512F, zmm512>::_ToMask(_VectorType_ _Vector) noexcept
{
    ...
    if constexpr (_Bits == 16) {
        return static_cast<_MaskType>(_mm512_cmp_epi32_mask(
            __intrin_bitcast<__m512i>(_Vector), _mm512_setzero_si512(), _MM_CMPINT_LT));
    }
    else if constexpr (_Bits == 8) {
        ...
        const auto _Low = _SimdToMask<arch::CpuFeature::AVX2, ymm256, _DesiredType_>(__intrin_bitcast<__m256d>(_Vector));
        const auto _High = _SimdToMask<arch::CpuFeature::AVX2, ymm256, _DesiredType_>(
            _mm512_extractf64x4_pd(__intrin_bitcast<__m512d>(_Vector), 1));
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept 
{
    ...
    if constexpr (_Bits == 16) {
        return __intrin_bitcast<_VectorType_>(_mm512_maskz_mov_epi32(_Mask, _mm512_set1_epi32(-1)));
    }
    if constexpr (_Bits == 8) {
        return __intrin_bitcast<_VectorType_>(_mm512_maskz_mov_epi64(_Mask, _mm512_set1_epi64(-1)));
    }
    else {
    ...
    _SimdConvertImplementation<arch::CpuFeature::AVX512BW, zmm512>::_ToMask(_VectorType_ _Vector) noexcept 
{
    ...
    if constexpr (_Bits == 64)
        return static_cast<_MaskType>(_mm512_movepi8_mask(__intrin_bitcast<__m512i>(_Vector)));

    else if constexpr (_Bits == 32)
        return static_cast<_MaskType>(_mm512_movepi16_mask(__intrin_bitcast<__m512i>(_Vector)));
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept
{
    ...
    if constexpr (_Bits == 64)
        return __intrin_bitcast<_VectorType_>(_mm512_movm_epi8(_Mask));

    else if constexpr (_Bits == 32)
        return __intrin_bitcast<_VectorType_>(_mm512_movm_epi16(_Mask));
    ...
    _SimdConvertImplementation<arch::CpuFeature::AVX512DQ, zmm512>::_ToMask(_VectorType_ _Vector) noexcept 
{
    ...
    if constexpr (_Bits == 64)
        ...
        return static_cast<_MaskType>(_mm512_movepi32_mask(__intrin_bitcast<__m512i>(_Vector)));

    else if constexpr (_Bits == 8)
        return static_cast<_MaskType>(_mm512_movepi64_mask(__intrin_bitcast<__m512i>(_Vector)));
}
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept 
{
    ...
    if constexpr (_Bits == 64)
        ...
        return __intrin_bitcast<_VectorType_>(_mm512_movm_epi32(_Mask));

    else if constexpr (_Bits == 8)
        return __intrin_bitcast<_VectorType_>(_mm512_movm_epi64(_Mask));
}
    ...
    _SimdConvertImplementation<arch::CpuFeature::AVX512VLBW, ymm256>::_ToMask(_VectorType_ _Vector) noexcept
{
    ...
    if constexpr (_Bits == 32)
        return static_cast<_MaskType>(_mm256_movepi8_mask(__intrin_bitcast<__m256i>(_Vector)));
    else if constexpr (_Bits == 16)
        return static_cast<_MaskType>(_mm256_movepi16_mask(__intrin_bitcast<__m256i>(_Vector)));
    else
    ...
    _SimdConvertImplementation<arch::CpuFeature::AVX512VLDQ, ymm256>::_ToMask(_VectorType_ _Vector) noexcept
{
    ...
    if constexpr (_Bits == 8)
        return static_cast<_MaskType>(_mm256_movepi32_mask(__intrin_bitcast<__m256i>(_Vector)));
    else if constexpr (_Bits == 4)
        return static_cast<_MaskType>(_mm256_movepi64_mask(__intrin_bitcast<__m256i>(_Vector)));
    else
        return _SimdToMask<arch::CpuFeature::AVX2, _RegisterPolicy, _DesiredType_>(_Vector);
}
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept 
{
    ...
    if constexpr (_Bits == 8)
        return __intrin_bitcast<_VectorType_>(_mm256_movm_epi32(_Mask));

    else if constexpr (_Bits == 4)
        return __intrin_bitcast<_VectorType_>(_mm256_movm_epi64(_Mask));
    ...
    _SimdConvertImplementation<arch::CpuFeature::AVX512VLBW, xmm128>::_ToMask(_VectorType_ _Vector) noexcept
{
    ...
    if constexpr (_Bits == 16)
        return static_cast<_MaskType>(_mm_movepi8_mask(__intrin_bitcast<__m128i>(_Vector)));
    else if constexpr (_Bits == 8)
        return static_cast<_MaskType>(_mm_movepi16_mask(__intrin_bitcast<__m128i>(_Vector)));
    else
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept 
{
    ...
    if constexpr (_Bits == 16)
        return __intrin_bitcast<_VectorType_>(_mm_movm_epi8(_Mask));
    else if constexpr (_Bits == 8)
        return __intrin_bitcast<_VectorType_>(_mm_movm_epi16(_Mask));
    else
    ...
    _SimdConvertImplementation<arch::CpuFeature::AVX512VLDQ, xmm128>::_ToMask(_VectorType_ _Vector) noexcept
{
    ...
    if constexpr (_Bits == 4)
        return static_cast<_MaskType>(_mm_movepi32_mask(__intrin_bitcast<__m128i>(_Vector)));
    else if constexpr (_Bits == 2)
        return static_cast<_MaskType>(_mm_movepi64_mask(__intrin_bitcast<__m128i>(_Vector)));
    else
        return _SimdToMask<arch::CpuFeature::SSE42, _RegisterPolicy, _DesiredType_>(_Vector);
}
    ...
    ::_ToVector(_Simd_mask_type<_DesiredType_> _Mask) noexcept 
{
    ...
    if constexpr (_Bits == 4)
        return __intrin_bitcast<_VectorType_>(_mm_movm_epi32(_Mask));
    else if constexpr (_Bits == 2)
        return __intrin_bitcast<_VectorType_>(_mm_movm_epi64(_Mask));
    else
        return _SimdToVector<arch::CpuFeature::SSE42, _RegisterPolicy, _DesiredType_>(_Mask);
}
