...
template <
    ...
    class       _ToType_,
    class       _FromType_>
simd_stl_always_inline _MaskTypeTo_ __mask_convert(_FromType_ __from) noexcept {
    if constexpr (std::is_integral_v<_FromType_> && std::is_integral_v<_ToType_>)
        ...
        return __simd_to_mask<_ISA_, _Width_, _DesiredType_>(__from);

    else if constexpr (std::is_integral_v<_FromType_> && __is_intrin_type_v<_ToType_>)
        return __simd_to_vector<_ISA_, _Width_, _ToType_, _DesiredType_>(__from);
}
