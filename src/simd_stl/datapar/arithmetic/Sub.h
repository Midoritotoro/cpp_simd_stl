...
struct _Simd_sub<arch::ISA::AVX512F, 512, _DesiredType_> {
		...
		_IntrinType_ __right) simd_stl_const_operator noexcept
	{
		if constexpr (__is_pd_v<_DesiredType_>) {
			...
			const auto __low	= _Simd_sub<arch::ISA::AVX2, ymm256, _DesiredType_>()(__intrin_bitcast<__m256i>(__left), __intrin_bitcast<__m256i>(__right));
			const auto __high	= _Simd_sub<arch::ISA::AVX2, ymm256, _DesiredType_>()(_mm512_extracti64x4_epi64(__intrin_bitcast<__m512i>(__left), 1),
